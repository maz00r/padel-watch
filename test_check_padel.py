...
class TestSsoBootstrap(unittest.TestCase):
    """Bootstrap: SESSION -> authorize -> code -> (jwt, rt) z useUnsafeRefreshToken."""

    LOGIN_URL = "https://api-global.decathlon.net/connect/oauth/authorize?client_id=x"

    def _http_error(self, code, location=None):
        hdrs = {"Location": location} if location else {}
        return urllib.error.HTTPError("u", code, "redir", hdrs, io.BytesIO(b""))

    def test_code_extracted_from_redirect(self):
        err = self._http_error(302, "https://go.decathlon.pl/logged-in?code=ABC123&state=")
        with mock.patch.object(cp.urllib.request, "build_opener") as bo:
            bo.return_value.open.side_effect = err
            code, e = cp.sso_authorize_code(self.LOGIN_URL, "SESSION=zyje")
        self.assertEqual(code, "ABC123")
        self.assertIsNone(e)

    def test_redirect_to_login_means_dead_cookie(self):
        err = self._http_error(302, "https://login.decathlon.net?client_id=x")
        with mock.patch.object(cp.urllib.request, "build_opener") as bo:
            bo.return_value.open.side_effect = err
            code, e = cp.sso_authorize_code(self.LOGIN_URL, "SESSION=martwa")
        self.assertIsNone(code)
        self.assertIn("nieważne", e, "musi jasno powiedzieć, że cookie SSO padło")

    def test_redirect_without_code(self):
        err = self._http_error(302, "https://go.decathlon.pl/logged-in?error=nope")
        with mock.patch.object(cp.urllib.request, "build_opener") as bo:
            bo.return_value.open.side_effect = err
            code, e = cp.sso_authorize_code(self.LOGIN_URL, "SESSION=x")
        self.assertIsNone(code)
        self.assertIn("brak `code`", e)

    def test_exchange_requests_refresh_token(self):
        seen = {}
        ...
        def fake_urlopen(req, timeout=30):
            seen["url"] = req.full_url
            seen["body"] = json.loads(req.data.decode("utf-8"))
            return _R(json.dumps({"jwt": "nowy.jwt.token", "rt": "refresh-1"}).encode())

        with mock.patch.object(cp.urllib.request, "urlopen", fake_urlopen):
            jwt, rt = cp.exchange_code_for_token("ABC123")
        self.assertEqual((jwt, rt), ("nowy.jwt.token", "refresh-1"))
        self.assertTrue(seen["url"].endswith("/auth/login/with-decathlon/token"))
        self.assertIs(seen["body"]["useUnsafeRefreshToken"], True,
                      "musimy JAWNIE prosić o rt — app webowa tego nie robi")
        self.assertEqual(seen["body"]["code"], "ABC123")

    def test_bootstrap_happy_path(self):
        with mock.patch.object(cp, "sso_login_url", lambda: self.LOGIN_URL), \
                mock.patch.object(cp, "sso_authorize_code", lambda u, c: ("ABC", None)), \
                mock.patch.object(cp, "exchange_code_for_token", lambda c: ("j1", "rt1")):
            jwt, rt, err = cp.bootstrap_decathlon_session("SESSION=zyje")
        self.assertEqual((jwt, rt, err), ("j1", "rt1", None))

    def test_bootstrap_without_cookie(self):
        jwt, rt, err = cp.bootstrap_decathlon_session("")
        self.assertIn("brak decathlon_sso_cookie", err)

    def test_ensure_falls_back_to_sso_when_refresh_fails(self):
        """Sedno: refresh pada 401 (jak u uzytkownika) -> ratuje nas bootstrap SSO."""
        expired = jwt_with_exp(int(datetime.now(timezone.utc).timestamp()) - 10)
        cfg = {"token": expired, "sso_cookie": "SESSION=zyje"}
        with mock.patch.object(cp, "refresh_decathlon_token",
                               side_effect=urllib.error.HTTPError("u", 401, "no", {}, io.BytesIO(b""))), \
                mock.patch.object(cp, "bootstrap_decathlon_session",
                                  lambda c: ("swiezy.jwt", "rt9", None)), \
                mock.patch.object(cp, "log"):
            token, err = cp.ensure_decathlon_token(cfg)
        self.assertIsNone(err)
        self.assertEqual(token, "swiezy.jwt")
        self.assertEqual(cfg["refresh_token"], "rt9", "rt musi trafic do cfg -> potem do stanu")

    def test_ensure_reports_when_both_paths_fail(self):
        expired = jwt_with_exp(int(datetime.now(timezone.utc).timestamp()) - 10)
        cfg = {"token": expired, "sso_cookie": "SESSION=martwa"}
        with mock.patch.object(cp, "refresh_decathlon_token",
                               side_effect=urllib.error.HTTPError("u", 401, "no", {}, io.BytesIO(b""))), \
                mock.patch.object(cp, "bootstrap_decathlon_session",
                                  lambda c: (None, None, "cookie nieważne")), \
                mock.patch.object(cp, "log"):
            token, err = cp.ensure_decathlon_token(cfg)
        self.assertIn("bootstrap SSO nieudany", err)
        self.assertTrue(any(m in err for m in cp.AUTH_FAILURE_MARKERS),
                        "musi byc rozpoznane jako blad auth (przerywa przebieg, alert)")

    def test_sso_not_tried_when_token_still_valid(self):
        valid = jwt_with_exp(int(datetime.now(timezone.utc).timestamp()) + 3600)
        cfg = {"token": valid, "sso_cookie": "SESSION=zyje"}
        with mock.patch.object(cp, "bootstrap_decathlon_session",
                               side_effect=AssertionError("nie wolno ruszac SSO przy zywym tokenie")):
            token, err = cp.ensure_decathlon_token(cfg)
        self.assertEqual(token, valid)
